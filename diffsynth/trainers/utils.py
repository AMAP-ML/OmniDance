...
class ModelLogger:
    ...
    def save_model(self, accelerator, model, file_name):
        ...
        if accelerator.is_main_process:
            ...
            copy_file_to_oss(path)


import os
import subprocess
def copy_file_to_oss(path):
    # 1. 确定源路径和目标父目录
    file = os.path.basename(path.rstrip("/"))
    # 注意：使用 cp 或 rsync 时，目标最好写父目录。它会自动把 file 文件夹塞进去。
    dst_parent = "/mnt/workspace/yangkaixing/ICML/OSS/weight/ti2v-stage1/"

    # 2. 确保挂载盘上的目标父目录存在，否则 cp/rsync 会报错
    os.makedirs(dst_parent, exist_ok=True)

    # 3. 构造异步命令 (强烈推荐用 rsync -a，稳！如果没装，改成 ["cp", "-r", path, dst_parent])
    # -a 归档模式，静默复制；去掉了 -v 和 -P，因为后台执行不需要看进度条，免得打乱你的训练打印日志
    cmd = ["rsync", "-a", path, dst_parent]

    # 4. 准备一个日志文件，用来记录后台拷贝的报错或信息（可选，但强烈建议）
    # 这样后台万一断了，你只要看这个 log 就能排查
    log_path = "./async_upload.log"
    log_file = open(log_path, "a")

    try:
        # 5. 【核心】：使用 Popen，不加 check=True，不加 wait()
        # 操作系统会立刻接管这个任务，Python 会在 0.001 秒内继续执行下一行
        subprocess.Popen(cmd, stdout=log_file, stderr=subprocess.STDOUT)
        print(f"🚀 [Async Upload] 正在后台静默上传: {file} -> {dst_parent}")
        print(f"📝 拷贝日志保存在: {os.path.abspath(log_path)}")
    except Exception as e:
        print(f"❌ [Async Upload] 启动后台上传失败: {e}")
...
def launch_training_task(
    ...
    start_lr, target_lr, warmup_steps = 1e-9, learning_rate, 1500
    optimizer = torch.optim.AdamW(model.trainable_modules(), lr=target_lr, weight_decay=weight_decay)
    warmup = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=start_lr / target_lr, end_factor=1.0, total_iters=warmup_steps)
    constant = torch.optim.lr_scheduler.ConstantLR(optimizer, factor=1.0, total_iters=10**9)
    scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer, schedulers=[warmup, constant], milestones=[warmup_steps])
