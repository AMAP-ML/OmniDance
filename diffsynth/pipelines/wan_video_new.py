...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        super().__init__(device=device, torch_dtype=torch_dtype, height_division_factor=16, width_division_factor=16, time_division_factor=4, time_division_remainder=1)
    ...
    def __call__(
        ...
        inputs_shared["mode"] = "MTI2V"
        inputs_shared["stage"] = stage


        # print(s2v_pose_video.shape, 'look look pose?')
        ...
        def get_text_cfg_scale(cfg_scale: float, timestep: float, t_max: float = 1000.0) -> float:
            t_norm = timestep / t_max          # 1.0 → 0.0
            cfg_end = 1.0
            scale = cfg_end + (cfg_scale - cfg_end) * t_norm
            return scale

        def get_audio_cfg_scale(cfg_scale: float, timestep: float, t_max: float = 1000.0) -> float:
            t_norm = timestep / t_max          # 1.0 → 0.0
            cfg_end = 2.0
            scale = cfg_scale + (cfg_end - cfg_scale) * (1 - t_norm)
            return scale
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if cfg_scale != 1.0:
                if cfg_merge:
                    ...
                    inputs_posi_textonly = copy.deepcopy(inputs_posi)
                    inputs_posi_textonly['audio_embeds'] = inputs_nega['audio_embeds']
                    inputs_posi_audioonly = copy.deepcopy(inputs_posi)
                    inputs_posi_audioonly['context'] = inputs_nega['anchor_context']
                    text_cfg_scale = get_text_cfg_scale(5.0, timestep)
                    audio_cfg_scale = get_audio_cfg_scale(4.0, timestep)
                    print('timestep', timestep, text_cfg_scale, audio_cfg_scale)

                    noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep)
                    noise_pred_posi_textonly = self.model_fn(**models, **inputs_shared, **inputs_posi_textonly, timestep=timestep)
                    noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep)
                    noise_pred = noise_pred_nega + text_cfg_scale * (noise_pred_posi_textonly - noise_pred_nega) + audio_cfg_scale * (noise_pred_posi - noise_pred_posi_textonly)
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        ...
        self.prompt_std = "色调艳丽，过曝，静态，细节模糊不清，字幕，风格，作品，画作，画面，静止，整体发灰，最差质量，低质量，JPEG压缩残留，丑陋的，残缺的，多余的手指，画得不好的手部，画得不好的脸部，畸形的，毁容的，形态畸形的肢体，手指融合，静止不动的画面，杂乱的背景，三条腿，背景人很多，倒着走"
        # self.prompt_std = 'Gesture is unclear. Unclear and blurry Face. Strange and twisted hands. Bad hands. Bad fingers. Unclear and blurry hands. '

    def process(self, pipe: WanVideoPipeline, prompt, positive, mode) -> dict:
        if (mode == 'MI2V' or mode == 'UnCond') and prompt != self.prompt_std:
            print('zero text')
            prompt = ''
            print(mode, prompt)
        else:
        ...
        anchor_emb = pipe.prompter.encode_prompt('一个人正在跳舞，并且视频画面清晰，人物脸部清晰，人物手部清晰，人物手指清晰，形体表现合理', positive=positive, device=pipe.device)
        
        return {"context": prompt_emb, 'anchor_context': anchor_emb}
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: WanVideoPipeline, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
        if audio_embeds is not None:
            audio_embeds = torch.from_numpy(audio_embeds).to(dtype=pipe.torch_dtype, device=pipe.device)
    ...
    def process(self, pipe: WanVideoPipeline, inputs_shared, inputs_posi, inputs_nega):
        ...
        #     return inputs_shared, inputs_posi, inputs_nega
        # 我们不用他的music encoder
        if (inputs_shared.get("input_audio") is None and inputs_shared.get("audio_embeds") is None):
            return inputs_shared, inputs_posi, inputs_nega
        ...
        mode = inputs_shared['mode']
        if mode == 'TI2V' or mode == 'PTI2V' or mode == 'PI2V' or mode == 'UnCond':
            print('zero audio')
            inputs_posi['audio_embeds'] = 0.0 * inputs_posi['audio_embeds']
        else:
            print('non-zero audio')
        return inputs_shared, inputs_posi, inputs_nega
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    s2v_pose_latents = dit.pose_patchify(s2v_pose_latents, control_camera_latents_input)
    ...
    # Patchify
    # print(x.shape, s2v_pose_latents.shape, 'gigi')

    f, h, w = x.shape[2:]
    x = rearrange(x, 'b c f h w -> b (f h w) c').contiguous()
    s2v_pose_latents = rearrange(s2v_pose_latents, 'b c f h w -> b (f h w) c').contiguous()
    audio_embeds = rearrange(audio_embeds, 't c -> 1 t c')
    # print(s2v_pose_latents.shape, 'gigi')

    s2v_pose_latents = dit.pose_emb(s2v_pose_latents)
    audio_embeds = dit.audio_emb(audio_embeds)
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vap is not None and block_id in vap.mot_layers_mapping:
                ...
                if use_gradient_checkpointing_offload:
                    ...
                    x = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(block),
                        x, context, audio_embeds, s2v_pose_latents, t_mod, freqs,
                        use_reentrant=False,
                    )
                else:
                    x = block(x, context, audio_embeds, s2v_pose_latents, t_mod, freqs)
