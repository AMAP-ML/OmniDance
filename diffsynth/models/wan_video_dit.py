...
def zero_init_state_dict(module: nn.Module) -> nn.Module:
...
class CrossAttention_Music(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6, has_image_input: bool = False):
        ...
        self.q = zero_init_state_dict(nn.Linear(dim, dim))
        self.o = zero_init_state_dict(nn.Linear(dim, dim))
        self.norm_q = zero_init_state_dict(RMSNorm(dim, eps=eps))

        self.k_audio = zero_init_state_dict(nn.Linear(dim, dim))
        self.v_audio = zero_init_state_dict(nn.Linear(dim, dim))
        self.norm_k_audio = zero_init_state_dict(RMSNorm(dim, eps=eps))  


    def forward(self, x: torch.Tensor, y: torch.Tensor, pose: torch.Tensor, audio: torch.Tensor):

        q = self.norm_q(self.q(x))

        # k_pose = self.norm_k_pose(self.k_pose(pose))
        # v_pose = self.v_pose(pose)
        # y = flash_attention(q, k_pose, v_pose, num_heads=self.num_heads)

        k_audio = self.norm_k_audio(self.k_audio(audio))
        v_audio = self.v_audio(audio)
        z = flash_attention(q, k_audio, v_audio, num_heads=self.num_heads)
...
class DiTBlock(nn.Module):
    def __init__(self, has_image_input: bool, dim: int, num_heads: int, ffn_dim: int, eps: float = 1e-6, layer: int = 0):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.ffn_dim = ffn_dim
        self.beta = ((layer + 1) / 30) ** 1.5

        self.self_attn = SelfAttention(dim, num_heads, eps)
        self.cross_attn = CrossAttention(dim, num_heads, eps, has_image_input=has_image_input)
        self.cross_attn_music = CrossAttention_Music(dim, num_heads, eps, has_image_input=has_image_input)
            
        self.norm1 = nn.LayerNorm(dim, eps=eps, elementwise_affine=False)
        self.norm2 = nn.LayerNorm(dim, eps=eps, elementwise_affine=False)
        self.norm3 = nn.LayerNorm(dim, eps=eps)

        self.norm4 = zero_init_state_dict(nn.LayerNorm(dim, eps=eps))
        
        self.ffn = nn.Sequential(nn.Linear(dim, ffn_dim), nn.GELU(approximate='tanh'), nn.Linear(ffn_dim, dim))
        self.modulation = nn.Parameter(torch.randn(1, 6, dim) / dim**0.5)
        self.gate = GateModule()

    def forward(self, x, context, audio_embeds, pose_latent, t_mod, freqs):
        # print(x.shape, context.shape, audio_embeds.shape, pose_latent.shape)
        ...
        x = x + self.cross_attn(self.norm3(x), context, pose_latent, audio_embeds)
        x = x + self.cross_attn_music(self.norm4(x), context, pose_latent, audio_embeds) * self.beta
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(has_image_input, dim, num_heads, ffn_dim, eps, layer)
        ...
        self.pose_patch_embedding = zero_init_state_dict(nn.Conv3d(in_dim, dim, kernel_size=patch_size, stride=patch_size)     )
        self.pose_emb = zero_init_state_dict(MLP(dim, dim, has_pos_emb=has_image_pos_emb))
        self.audio_emb = zero_init_state_dict(MLP(1024, dim, has_pos_emb=has_image_pos_emb))
    ...
    def pose_patchify(self, x: torch.Tensor, control_camera_latents_input: Optional[torch.Tensor] = None):
...
class WanModelStateDictConverter:
    ...
    def from_civitai(self, state_dict):
        ...
        print('dudud')
        print(hash_state_dict_keys(state_dict))
        ...
        elif hash_state_dict_keys(state_dict) == "bedf7e4c94f63a223785824a37ccf8ad":
            # ours
            config = {
                "has_image_input": False,
                "patch_size": [1, 2, 2],
